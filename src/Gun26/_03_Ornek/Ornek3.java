package Gun26._03_Ornek;

public class Ornek3 {
    // 1.Adım  :ayrı dosyada olmak üzere Musteri isimli (fields: name) isimli bir class tanımlayınız.
    //2.Adım  :ayrı bir dosyada ElektrikHesabi isimli (fields : totalKw, rate(double), bill)
    //3.Adım  :Customer a bir ElektrikHesabi ekleyiniz.
    //4.Adım  :1 tane müşteri oluşturunuz, Hesabı u dahil;
    //5.Adım  :Musteriye ait her ay tüketilen enerjiyi toplayacak bir metod yazınız.
    //         tuketimEkle olasun her verilen aylık rakamı toplayarak biriktirsin.
    //6.Adım  :Ödenecek rakamı  toplam tüketimi  oran(0.7) ile çarparak hesaplatınız yazdırınız.
    //ipucu : hesabı olşuturduktan sonra tuketimEkle metoduna 2,3 ay tüketim ekletiniz, faturayı
    //        ondan sonra toplam oalrak çıkartınız

    public static void main(String[] args) {

        Musteri musteri=new Musteri();
        musteri.name="Alperen";

        musteri.elektrikHesabi=new ElektrikHesabi();
        musteri.elektrikHesabi.tuketimEkle(100);//ocak ayi
        System.out.println("musteri.elektrikHesabi.toplamtuketim = " + musteri.elektrikHesabi.toplamtuketim);
        musteri.elektrikHesabi.tuketimEkle(150);//subat ayi
        System.out.println("musteri.elektrikHesabi.toplamtuketim = " + musteri.elektrikHesabi.toplamtuketim);

        System.out.println("musteri.elektrikHesabi.odenecekRakam() = " + musteri.elektrikHesabi.odenecekRakam());
        System.out.println("musteri.elektrikHesabi.bill = " + musteri.elektrikHesabi.bill);


    }
}
