package Gun34._02_DigerIslemler;

import java.time.LocalDate;
import java.time.LocalTime;
import java.time.format.DateTimeFormatter;

public class _02_StringToLocalDateAndTime {
    public static void main(String[] args) {

        //Kullanicidan alinan ve String durumunda olan bir time veya tarih
        //bilgisinin time veya tarih degiskenine cevrilmesi.

        String strDate= "01 25 2020"; //25 Ocak 2020;

        //String Degeri Tarihe Cevirmek Icin Belirlenen Format
        DateTimeFormatter format = DateTimeFormatter.ofPattern("MM dd yyyy");
        //String formati bu formata uygun olmali parse ile cevrilir.
        LocalDate tarih = LocalDate.parse(strDate, format);
        System.out.println("tarih = " + tarih);

        String strTime = "12:49";
        LocalTime saat = LocalTime.parse(strTime);
        System.out.println("saat = " + saat);

    }
}
